...
END:VCARD
	...
	<cffunction name="create">
		...
		<cfif handbookperson.save()>
			...
			<cfif isAgbmAdmin>
				<cfset redirectTo(controller="handbook.agbm-info", action="add", key=handbookperson.id)>
			<cfelse>
...
<!---Special Actions (not sure what to do with these--->
	...
	<cffunction name="sendToHandbookPeople">
		...
		<cfset churchEmails1 = model("Handbookorganization").findAll(where="statusid IN (1,8,3,4,2,9,10,11)", select="email as emailsend, name as name", maxrows=maxrows, include="Handbookstate")>
