...
<!----------------------------------------->	
	function findFocus(required struct params) {
		...
		loc.handbookOrganizations = model("Handbookorganization").findAll(select="name AS 'fname', email, phone, region, focusretreat", where="focusretreat = '#loc.retreat#'", include="Handbookdistrict", order="email" )
		...
		loc.allhandbook = combineTwoQueries(loc.handbookOrganizations,loc.handbookpeople)
		loc.allpeople = combineTwoQueries(loc.allhandbook, loc.focuspeople)
