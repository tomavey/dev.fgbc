...
<div style='mso-pagination:widow-orphan lines-together'>
		  ...
		  <cfif params.action NEQ "bluepages" and params.action NEQ "handbookinfo" or isDefined("params.showedit")>
			...
			<cfif !isAgbm(id) and gotRights("office")>						
				#linkto(text='<i class="icon-trash"></i>', controller="handbook.People", action="delete", method="delete", key=id, class="tooltipside ajaxdelete", title="Delete #fname# #lname#", onclick="confirm('Are you sure?')", target="_blank", class="noAjax")#
			</cfif>  
		  </cfif>
