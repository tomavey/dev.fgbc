...
<h1>Showing user</h1>
					...
					<p><label>Groups:</label>
						<cfloop query="groups">
							<cftry>
							#linkTo(text=title, controller="auth.groups", action="show", key=auth_groupsid)#-#linkTo(text="x", controller="auth.users", action="removeFromGroup", params="groupId=#auth_groupsid#&userid=#params.key#")#
								<cfcatch></cfcatch>
							</cftry>
						</cfloop>
